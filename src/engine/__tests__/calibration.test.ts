...
describe("calibration to 2020", () => {
  ...
  it("neutral (do-nothing) play lands on Biden 306 / Trump 232", () => {
    const game = createGame({ seed: "calibration", playerCandidate: "biden" });
    const result = computeResult(game);
    expect(result.electoralVotes.biden).toBe(306);
    expect(result.electoralVotes.trump).toBe(232);
    expect(result.winner).toBe("biden");
  });

  it("neutral national two-party share is Biden ~52% (real: 52.3%)", () => {
    const game = createGame({ seed: "calibration" });
    const result = computeResult(game);
    expect(result.popularShare.biden).toBeGreaterThan(0.515);
    expect(result.popularShare.biden).toBeLessThan(0.53);
  });
  ...
  it("1000 noisy neutral sims center near Biden ~306 EV", () => {
    ...
    for (let r = 0; r < runs; r++) {
      ...
      const result = computeResult(game);
      evSum += result.electoralVotes.biden;
      if (result.electoralVotes.biden >= 270) bidenWins++;
      for (const id of ["AZ", "GA", "WI", "PA", "NV", "NC", "FL"]) {
        const sr = result.stateResults.find((s) => s.stateId === id)!;
        if (sr.winner === "trump") flips.add(id);
      }
    }
    ...
    expect(bidenWins).toBeGreaterThan(620);
    expect(bidenWins).toBeLessThan(950);
